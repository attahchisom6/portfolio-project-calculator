    ...
    for (const item of funcs) {
      if (expression.includes(item)) {
        ...
        if (isFloatParsable(prevItem)) {
          temp = prevItem;
          prevItem = prevItem + '*';
          prevIndex = expression.indexOf(temp);
          refinedExpression = expression.slice(0, prevIndex) + prevItem + expression.slice(prevIndex + temp.length);
        } else {
          refinedExpression = expression;
        }
        temp = ""; //resetting the value stored in temp

        if (!/\((\w+)\)/.test(nextItem)) {
          temp = nextItem;
          nextIndex = expression.indexOf(temp);
          nextItem = '(' + nextItem + ')';
          console.log('new next item:', nextItem);
          refinedExpression = expression.slice(0, nextIndex) + nextItem + expression.slice(nextIndex + temp.length);
        } else {
          refinedExpression = expression;
        }
        temp = "";
